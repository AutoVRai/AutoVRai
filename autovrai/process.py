...
def process_image_directory(config, model: autovrai.ZoeDepth_DepthModel):
    ...
    filenames = autovrai.find_filenames(config['input-source'], config['input-patterns'])
    ...
    for i in trange(filenames.__len__()):
        ...
        if config['precision-type'] == 'factor':
            set_precision(model, image.width * config['precision-width'], image.height * config['precision-height'])
        ...
        if config['output-depthraw'] != '':
            autovrai.ZoeDepth_save_raw_16bit(depth, os.path.join(config['output-depthraw'], filepath))
